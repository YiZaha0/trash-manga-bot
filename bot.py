...
async def chapter_click(client, data, chat_id, custom_caption=""):
    ...
    async with locks[chat_id]:
        ...
        if download:
            ...
            ch_name = clean(f'{chapter.name.replace("Chapter", "Ch -")} {clean(chapter.manga.name, 25)}', 45) if chapter.client != "Manhwa18" else clean(f'{chapter.name.replace("Chapter", "Ch -")} {clean(chapter.manga.name, 25)}', 31) + " @Adult_Mangas"
            try:

...
async def chapter_click(client, data, chat_id, custom_caption=""):
    ...
    async with locks[chat_id]:
        ...
        if download:
            ...
            ch_name = clean(f'{chapter.name.replace("Chapter", "Ch -")} {clean(chapter.manga.name, 25)}', 60) if chapter.client.name != "Manhwa18" else clean(f'{chapter.name.replace("Chapter", "Ch -")} {clean(chapter.manga.name, 25)}', 45) + " @Adult_Mangas"
            try:

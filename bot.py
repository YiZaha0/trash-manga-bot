...
async def manga_click(client, callback: CallbackQuery, pagination: Pagination = None):
    ...
    prev = [InlineKeyboardButton('<<', f'{pagination.id}_{pagination.page - 1}' if pagination.page != 0 else f'{pagination.id}_{len(all_results)-1}')]
    next_ = [InlineKeyboardButton('>>', f'{pagination.id}_{pagination.page + 1}' if pagination.page != (len(all_results)-1) else f'{pagination.id}_0')]
    footer = [prev + next_] 
...
async def chapter_click(client, data, chat_id, custom_caption=""):
    ...
    async with locks[chat_id]:
        ...
        if download:
            ...
            ch_name = clean(f'{chapter.name.replace("Chapter", "Ch -")} {clean(chapter.manga.name, 25)}', 45) if chapter.client.name != "Manhwa18" else clean(f'{chapter.name.replace("Chapter", "Ch -")} {clean(chapter.manga.name, 25)}', 31) + " @Adult_Mangas"
            try:
